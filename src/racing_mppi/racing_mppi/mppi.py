...
class MPPI(nn.Module):
    ...
    def __init__(
        ...
        # self._du_min = du_min.clone().detach().to(self._device, self._dtype)
        # self._du_max = du_max.clone().detach().to(self._device, self._dtype)
    ...
    def forward(
        ...
        # self._action_noises = torch.clamp(self._action_noises, [-100.0, -100.0], [100.0, 100.0])
