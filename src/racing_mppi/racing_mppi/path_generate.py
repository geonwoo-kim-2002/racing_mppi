...
def make_csv_paths(csv_file: str, DL=0.1, offset=False) -> np.ndarray:
    """make_csv_paths
    Input parameters:
    ...
    # interpolate paths
    # right_path, left_path = make_side_lane(circuit_path, df.loc[:, 'w_tr_right_m'].to_numpy(), df.loc[:, 'w_tr_left_m'].to_numpy())
...
def make_side_lane(road: np.ndarray, right_lane_width: np.ndarray, left_lane_width: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """make_side_lane
    Input parameters:
    ...
    """
    right_lane_x = right_lane_width/2*np.cos(road[:,2]-np.pi/2) +road[:,0]
    right_lane_y = right_lane_width/2*np.sin(road[:,2]-np.pi/2) +road[:,1]
    right_lane_pos = np.stack((right_lane_x, right_lane_y), axis=1)

    left_lane_x = left_lane_width/2*np.cos(road[:,2]+np.pi/2) +road[:,0]
    left_lane_y = left_lane_width/2*np.sin(road[:,2]+np.pi/2) +road[:,1]
